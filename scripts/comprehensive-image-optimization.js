...
async function main() {
  ...
  for (const image of images) {
    const originalSize = (await stat(image)).size;
    totalOriginalSize += originalSize;

    await optimizeImage(image);

    // Calculate optimized size (WebP version)
    const ext = path.extname(image).toLowerCase();
    const basename = path.basename(image, ext);
    const webpPath = path.join(path.dirname(image), `${basename}.webp`);

    if (fs.existsSync(webpPath)) {
      const optimizedSize = (await stat(webpPath)).size;
